LOKI TRAFFIC LAB 3.2.0 - UBUNTU / LINUX
=======================================
...
From a local checkout/release directory:
  ...
  sudo bash ./Linux/bootstrap.sh --archive ./Linux/releases/LokiTrafficLab-linux-x64-3.2.0.tar.gz

For a hosted release, bootstrap also supports a one-line installation:

  curl -fsSL https://YOUR-HOST/bootstrap.sh -o /tmp/tlab-bootstrap.sh && echo 'BOOTSTRAP_SHA256  /tmp/tlab-bootstrap.sh' | sha256sum -c - && sudo bash /tmp/tlab-bootstrap.sh --url https://YOUR-HOST/LokiTrafficLab-linux-x64-3.2.0.tar.gz
...
From PowerShell at the repository root:
  ...
  & '.\traffic-lab\Linux\build-linux.ps1' -RuntimeIdentifier linux-x64 -OutputDirectory 'Linux\releases\3.2.0' -Archive
